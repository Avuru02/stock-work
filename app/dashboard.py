...
from config import SCORE_WEIGHTS, SECTOR_ETFS, SECTOR_LABELS
...
def load_payload(nonce: int, force: bool, layout_version: int = 3) -> dict:
...
def rrg_figure(latest: pd.DataFrame, trails: pd.DataFrame, title: str = "Relative Rotation Graph (RS-Ratio vs RS-Momentum, 100 = SPY)") -> go.Figure:
    fig = go.Figure()
    x_vals = latest["rs_ratio"]
    y_vals = latest["rs_momentum"]
    pad = 1.5
    ...
    if not trails.empty:
        for etf, trail in trails.groupby("etf"):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=trail["rs_momentum"],
                    mode="lines",
                    line=dict(color="#6E7681", width=1),
                    hoverinfo="skip",
                    showlegend=False,
                )
            )
    ...
    fig.add_trace(
        go.Scatter(
            ...
            marker=dict(
                size=12,
                color=[colors.get(q, "#8B949E") for q in latest["quadrant"]],
            ),
            customdata=latest[["sector", "quadrant"]],
            hovertemplate="%{customdata[0]} (%{text})<br>RS-Ratio: %{x:.2f}<br>RS-Momentum: %{y:.2f}<br>%{customdata[1]}<extra></extra>",
            showlegend=False,
        )
    )
    fig.update_layout(
...
def main() -> None:
    ...
    st.caption(
        ...
        "Money-flow figures are volume/price proxies, not ETF creations or 13F allocations."
    )
    ...
    payload = load_payload(st.session_state.data_nonce, st.session_state.force_refresh, 3)
    ...
    with tab_sectors:
        ...
        with left:
            st.plotly_chart(rrg_figure(payload["rrg"], payload["rrg_trails"]), width="stretch")
        with right:
            ...
            st.caption(
                "Score blends 1M relative-strength rank, RRG momentum, relative dollar volume, "
                "Chaikin money flow, and % of names above the 50-day average. "
                "Rel $ vol is last-session dollar volume vs the 20-day average."
            )
        ...
        b1.metric("Rotation score", _fmt_num(row["score"], 0), str(row["flow_bucket"]))
        b2.metric("RRG quadrant", str(row["quadrant"]))
        b3.metric("% above 50DMA", _fmt_pct(row["pct_above_50dma"], 0))
        b4.metric("Names up 1M", _fmt_pct(row.get("pct_up_1m"), 0))
        b5.metric("20d highs / lows", f"{int(row.get('new_highs_20', 0))} / {int(row.get('new_lows_20', 0))}")

        m1, m2, m3 = st.columns(3)
        m1.metric("ETF CMF (20d)", _fmt_num(row.get("cmf"), 2))
        m2.metric("ETF rel $ volume", _fmt_num(row.get("rel_dollar_volume"), 2) + "x")
        stock_dvol = row["stock_dollar_volume"] if "stock_dollar_volume" in scored.columns else None
        m3.metric("Constituent $ volume", _fmt_billions(stock_dvol))
    ...
    with tab_industries:
        st.caption(
            ...
            "Scores are ranked among these industry groups only, not against the 11 GICS sectors."
        )
        if industry_scored.empty:
            ...
            with i_left:
                st.plotly_chart(
                    rrg_figure(
                        industries["rrg"],
                        industries["rrg_trails"],
                        title="Industry RRG (equal-weight baskets vs SPY)",
                    ),
                    width="stretch",
                )
            with i_right:
            ...
            n1.metric("Rotation score", _fmt_num(irow["score"], 0), str(irow["flow_bucket"]))
            n2.metric("RRG quadrant", str(irow["quadrant"]))
            n3.metric("Parent sector", str(irow.get("parent_sector", "—")))
            n4.metric("Names", str(int(irow.get("n_stocks", 0))))
